...
static jlong Typeface_updateEmojiFonts(JNIEnv *env, jobject, jlongArray familyArray) {
    ScopedLongArrayRO families(env, familyArray);
    TypefaceImpl_updateEmojiFonts(families.get(), families.size());
    return 0;
}

static void Typeface_clearEmojiFonts(JNIEnv* env) {
    return TypefaceImpl_clearEmojiFonts();
}
