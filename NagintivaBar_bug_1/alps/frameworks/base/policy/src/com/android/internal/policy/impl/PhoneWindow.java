...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    private int area_y_min = 1900;//the response area of action down for shortcut bar
    private int area_y_max = 1920;
    // cqf add MOPLUES-18 20151203 end
    ...
    public PhoneWindow(Context context) {
        ...
        if(windowService != null){
            ...
            Log.i(TAG,"PhoneWindow display x--" + display_x+"   y--"+display_y);
        }	
        // cqf add MOPLUES-18 20151203 end
    }
    ...
    private final class DecorView extends FrameLayout implements RootViewSurfaceTaker {
        ...
        public boolean onInterceptTouchEvent(MotionEvent event) {
            ...
            //cqf add POPLUSVNMS-49 20151223 start
            boolean flag = false;
            Configuration mConfiguration  = getContext().getResources().getConfiguration();
            int  ori = mConfiguration.orientation ;		
            //cqf add POPLUSVNMS-49 20151223 end

            if (action == MotionEvent.ACTION_DOWN) {
                int y = (int)event.getY();
                int x = (int)event.getX();

                /*Old: if(y>area_y_min&&y<area_y_max){
                    SHORTCUT_INFO = 1;
                    mX=x;
                    mY=y;		
                }*/
			...
			//cqf add POPLUSVNMS-49 20151223 end
                    if(flag){//((mY-y1)>30) {  //cqf modify POPLUSVNMS-49 20151223 
				Intent intent = new Intent();		
				intent.setAction(MALATA_UPDATE_NAVIGATIONBAR_ACTION);
                            mContext.sendBroadcast(intent);
				Log.i(TAG, "malata sendBroadcast MALATA_UPDATE_NAVIGATIONBAR_ACTION ");			
			       mX = 0;
                            mY = 0;
                            SHORTCUT_INFO = 0;
				flag = false;
