...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
    ...
    private boolean mHideNavigationBar = false; //cqf add MOPLUES-18 20151203
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        //chb add for factorytest begin 2015-11-5 10:53:51
        IntentFilter factoryTestFilter = new IntentFilter();
        factoryTestFilter.addAction(FACTORYTEST_CLOSERECENTAPPS_ACTION);
        factoryTestFilter.addAction(FACTORYTEST_STARTRECENTAPPS_ACTION);
        context.registerReceiver(mFactoryTestReceiver, factoryTestFilter);
        //chb add for factorytest end 2015-11-5 10:54:03
        ...
        //cqf add MOPLUES-18 20151203 start
        IntentFilter mNavigationBarfilter = new IntentFilter();
        mNavigationBarfilter.addAction(MALATA_UPDATE_NAVIGATIONBAR_ACTION);       
        mContext.registerReceiver(updateNavigationBarReceiver, mNavigationBarfilter);
        //cqf add MOPLUES-18 20151203 end
    ...
    private View.OnClickListener mRecentsClickListener = new View.OnClickListener() {
        public void onClick(View v) {
        //chb add for factorytest begin 2015-11-5 10:55:54
               Intent mIntent = new Intent();
               mIntent.setAction(FACTORYTEST_MENU_DISABLE_ACTION);
               mContext.sendBroadcast(mIntent);
               ActivityManager activityManager = (ActivityManager) mContext
                               .getSystemService(Context.ACTIVITY_SERVICE);
               if (mIsRecentAppsDisable
                               && activityManager.getRunningTasks(1).get(0).topActivity
                                  .getClassName().equals(
                                "com.mlt.factorytest.ItemTestActivity")) {
               }
               else{
                      awakenDreams();
                      toggleRecentApps();
        ...
        //chb add for factorytest end 2015-11-5 10:55:51
    };
    ...
    View.OnTouchListener mHomeActionListener = new View.OnTouchListener() {
        public boolean onTouch(View v, MotionEvent event) {
            //chb add for factorytest begin 2015-11-5 10:58:53
            ActivityManager activityManager = (ActivityManager) mContext
                            .getSystemService(Context.ACTIVITY_SERVICE);
            boolean mIsFactoryTest = mIsRecentAppsDisable
                             && activityManager.getRunningTasks(1).get(0).topActivity
                            .getClassName().equals("com.mlt.factorytest.ItemTestActivity");
            //chb add for factorytest end 2015-11-5 10:58:57
            switch(event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                //chb add for factorytest begin 2015-11-5 10:58:59
                if(mIsFactoryTest){
                    Intent mIntent = new Intent();
                    mIntent.setAction(FACTORYTEST_HOME_DISABLE_ACTION);
                    mContext.sendBroadcast(mIntent);
                    v.setPressed(true);
                    KeyButtonView keyButtonView = (KeyButtonView) v;
                    keyButtonView.sendEvent(KeyEvent.ACTION_DOWN, 0);
                }           
                //chb add for factorytest end 2015-11-5 10:59:00
            ...
            //chb add for factorytest begin 2015-11-5 10:59:54
            if(mIsFactoryTest){
                 v.setPressed(false);
            }
            //chb add for factorytest end 2015-11-5 10:59:57
                mHandler.removeCallbacks(mShowSearchPanel);
                awakenDreams();
            break;
        }
        //chb add for factorytest begin 2015-11-5 11:01:36
        if (mIsFactoryTest) {
               return true;
        } else{
                return false;
        }
        //chb add for factorytest end 2015-11-5 11:01:45
        }
    };
    ...
    private void prepareNavigationBarView() {
	 ...
	 //cqf add MOPLUES-18 20151203 start
        mNavigationBarView.getHideNavigationBarButton().setOnClickListener(mHideNavigationBarButtonClickListener);
	 //cqf add MOPLUES-18 20151203 end
	/// M: add for multi window @{
        if(RecentsActivity.FLOAT_WINDOW_SUPPORT){
    ...
    //cqf add MOPLUES-18 20151203 add start
    private View.OnClickListener mHideNavigationBarButtonClickListener = new View.OnClickListener() {
        @Override
        public void onClick(View view) {
           
	     if(mHideNavigationBarButtonClickListener != null){		
    ...
    void makeExpandedVisible(boolean force) {
	 ...
	 //old:if (mNavigationBarView != null)	 //cqf del MOPLUES-18 20151203
        if (mNavigationBarView != null && !mHideNavigationBar)  //cqf add MOPLUES-18 20151203
    ...
    //chb add for factorytest begin 2015-11-5 11:03:59
    ...
    private boolean mIsRecentAppsDisable = false;
    BroadcastReceiver mFactoryTestReceiver  = new BroadcastReceiver() {
       @Override
       public void onReceive(Context mContext, Intent mIntent) {
               String action = mIntent.getAction();
               Log.i(TAG, "mFactoryTestReceiver receive a message:"+action);
               if (action.equals(FACTORYTEST_CLOSERECENTAPPS_ACTION)) {
                       mIsRecentAppsDisable = true;
               }else if (action.equals(FACTORYTEST_STARTRECENTAPPS_ACTION)) {
                       mIsRecentAppsDisable = false;
               }
       }
    };
    //chb add for factorytest end 2015-11-5 11:03:53

//cqf add MOPLUES-18 20151203 start

   private final static String MALATA_UPDATE_NAVIGATIONBAR_ACTION = "android.intent.action.MALATA_UPDATE_NAVIGATIONBAR_ACTION";
	
   private BroadcastReceiver updateNavigationBarReceiver = new BroadcastReceiver() {
    @Override
    public void onReceive(final Context context, final Intent intent) {

        Log.i(TAG, "malata PhoneStatusBar received " + intent.getAction());        
        addNavigationBar();
...
//            Log.d(TAG, "size=" + newIconSize + " padding=" + newIconHPadding);
    ...
    public void destroy() {
        ...
        if (mStatusBarWindow != null) {
			...
			mStatusBarWindowManager.hasAddView = false;
        }
        if (mNavigationBarView != null) {
        ...
        //chb add for factorytest begin 2015-11-5 11:05:03
        mContext.unregisterReceiver(mFactoryTestReceiver);
        //chb add for factorytest end 2015-11-5 11:05:05
	mContext.unregisterReceiver(updateNavigationBarReceiver); //cqf add MOPLUES-18 20151203
    }
