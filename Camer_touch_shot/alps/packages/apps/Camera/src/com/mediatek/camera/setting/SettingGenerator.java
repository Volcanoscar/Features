...
public class SettingGenerator {
    ...
    private boolean filterPreference(final ListPreference preference) {
        ...
        case SettingConstants.ROW_SETTING_TOUCH_SHOT:
            if (!FeatureSwitcher.isTouchShotSupport()) {
                removePreference = true;
            }
            break;
    ...
    private boolean buildFaceBeautyPreference(String key, ListPreference fbPreference, int row) {
        ...
        if (max == 0 && min == 0) {
            Log.w(TAG, "key:" + key + ", min and max is 0");
            return true;
        }
        
        String defaultValue = ParametersHelper.getDefaultValue(parameters, key);
        Log.i(TAG, "key:" + key + ", defaultValue:" + defaultValue + ", max:" + max + ", min:" + min);
        CharSequence[] faceBeautyValue = new CharSequence[] {
                String.valueOf(min),
                String.valueOf((max + min) / 2),
                String.valueOf(max),
        };
        fbPreference.setEntryValues(faceBeautyValue);
        if (defaultValue == null) {
            defaultValue = String.valueOf((max + min) / 2);
        }
        CharSequence[] defaultValues = new CharSequence[1];
        defaultValues[0] = defaultValue;
        fbPreference.setDefaultValues(defaultValues);
        return false;
    }
