...
public class PhotoActor extends CameraActor implements FocusManager.Listener,
    ...
    private boolean mTouchShot = false;
    private View mSaveView = null;
    private int mSaveX = 0;
    private int mSaveY = 0;
    ...
    private SelfTimerListener mSelfTimerListener = new SelfTimerListener() {
        ...
        public void onTimerTimeout() {
            ...
            /*liuhao modified for touch shot 20150126 begin*/
            if (isTouchShotOn() && mTouchShot) {
                mOnSingleTapListener.onSingleTapUp(mSaveView, mSaveX, mSaveY);
            } else {
                onShutterButtonClick(null);
                mModuleManager.onSelfTimerState(false);
            }
            /*liuhao modified for touch shot 20150126 end*/
        }
    };
    ...
    private boolean isTouchShotOn() {
        return FeatureSwitcher.isTouchShotSupport()
                && mCameraActivity.getListPreference(SettingConstants.KEY_TOUCH_SHOT).isEnabled()
                && mCameraActivity.getISettingCtrl()
                        .getSettingValue(SettingConstants.KEY_TOUCH_SHOT).equals("on");
    }
    /*liuhao added for touch shot 20150126 end*/
}
