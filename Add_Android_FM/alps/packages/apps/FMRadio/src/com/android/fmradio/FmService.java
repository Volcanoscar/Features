...
// laiyang for LFZSBDZT-42:headphone 20151126 start
import android.content.ComponentName;
import android.media.RemoteControlClient;
import android.media.RemoteControlClient.MetadataEditor;
// laiyang for LFZSBDZT-42:headphone 20151126 end
...
public class FmService extends Service implements FmRecorder.OnRecorderStateChangedListener {
    ...
    private FmMediaButtonIntentReceiver mMediaButtonReceiver;
    // laiyang for LFZSBDZT-42:headphone 20151126 end
    ...
    ///aoran add for L1,add FM channel list for defaults.@{
    private int defaultChannels[] = new int[] {880, 884, 892, 896, 1000};
    private String defaultChanelNames[] = new String[] {"Radio Foorti", "Radio Amar", "ABC Radio", "Radio Today", "BBC"};
    ///aoran add for L1,add FM channel list for defaults.@}
    ...
    private class FmServiceBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ((SOUND_POWER_DOWN_MSG.equals(action) && CMDPAUSE.equals(command))) {
                 ...
                 */
                Log.d(TAG, "isActivityForeground = " + isActivityForeground()
                      + ", mValueHeadSetPlug = " + mValueHeadSetPlug);
    ...
    private void setForceUse(boolean isSpeaker) {
        ...
        Log.d(TAG, "setForceUse, mForcedUseForMedia = " + mForcedUseForMedia);
        ...
        public void run() {
            try {
                ...
                while (!Thread.interrupted()) {
                		Log.d(TAG, "RenderThread: run, isRender = " + isRender());
                    if (isRender()) {
    ...
    private boolean powerUp(float frequency) {
        ...
        if (!requestAudioFocus()) {
            // activity used for update power down menu
        ...
        // laiyang for LFZSBDZT-42:headphone 20151126
    ...
    private boolean powerDown() {
    		Log.d(TAG, "powerDown, mPowerStatus = " + mPowerStatus);
    ...
    public void seekStationAsync(float frequency, boolean isUp) {
    		Log.d(TAG, "seekStationAsync, frequency = " + frequency
    				  + ", isUp = " + isUp);
        mFmServiceHandler.removeMessages(FmListener.MSGID_SEEK_FINISHED);
    ...
    private float seekStation(float frequency, boolean isUp) {
    		Log.d(TAG, "seekStation, frequency = " + frequency
    				  + ", isUp = " + isUp);
        if (mPowerStatus != POWER_UP) {
            return -1;
        }
    ...
    public boolean stopScan() {	
    		Log.d(TAG, "startScan, mPowerStatus = " + mPowerStatus);
        if (mPowerStatus != POWER_UP) {
            return false;
        }
    ...
    public void initService(int iCurrentStation) {
        ...
        ///aoran add for L1,add FM channel list for defaults.@{
        insertDefaultStation(defaultChannels,defaultChanelNames);
        ///aoran add for L1,add FM channel list for defaults.@}
    }

    ///aoran add for L1,add FM channel list for defaults.@{
    private int[] insertDefaultStation(int[] channels, String[] channelNames) {
        Log.d(TAG, "insertDefaultStation.firstValidChannel:" + Arrays.toString(channels));
        ...
        if (null != channels) {
            ...
            for (int i = 0; i < size; i++) {
                if (FmUtils.isValidStation(channels[i])) {
                    if (0 == channelNum) {
                        firstValidChannel = channels[i];
                    }
                    if (!FmStation.isDefaultStation(mContext, channels[i])) {
                        ops.add(ContentProviderOperation.newInsert(Station.CONTENT_URI)
                                .withValue(Station.STATION_NAME, channelNames[i])
                                .withValue(Station.FREQUENCY, channels[i])
                                .build());
                    }
                    channelNum++;
                }
            ...
            try {
                mContext.getContentResolver().applyBatch(FmStation.AUTHORITY, ops);
        ...
        Log.d(TAG, "insertDefaultStation.firstValidChannel:" + firstValidChannel + ",channelNum:" + channelNum);
    ...
    public void onCreate() {
        ...
        createRenderThread();
        // laiyang for LFZSBDZT-42:headphone 20151126 start
        mMediaButtonReceiver = new FmMediaButtonIntentReceiver();
        IntentFilter filter = new IntentFilter();
        filter.addAction("android.intent.action.MEDIA_BUTTON");
        filter.setPriority(1000);
        this.registerReceiver(mMediaButtonReceiver, filter);
    ...
    private class FmOnAudioPortUpdateListener implements OnAudioPortUpdateListener {
        ...
        public void onAudioPatchListUpdate(AudioPatch[] patchList) {
        		Log.d(TAG, "onAudioPatchListUpdate, mAudioPatch = " + mAudioPatch
        		      + ", mIsRender = " + mIsRender);
    ...
    private void setLRText(String lrtText) {
    		Log.d(TAG, "setLRText, string = " + lrtText);
    ...
    private void enableFmAudio(boolean enable) {
    		Log.d(TAG, "enableFmAudio, enable = " + enable);
        if (enable) {
    ...
    private boolean isPatchMixerToEarphone(ArrayList<AudioPatch> patches) {
        ...
        for (AudioPatch patch : patches) {
            ...
            Log.d(TAG, "isPatchMixerToEarphone, sinks num: " + sinks.length);
            if (sinks.length > 1) {
                continue;
            }
        ...
        Log.d(TAG, "isPatchMixerToEarphone: false " + deviceCount + ", " + deviceEarphoneCount);
        return false;
    }
    ...
    private boolean isPatchMixerToSpeaker(ArrayList<AudioPatch> patches) {
        ...
        for (AudioPatch patch : patches) {
            AudioPortConfig[] sources = patch.sources();
            AudioPortConfig[] sinks = patch.sinks();
            Log.d(TAG, "isPatchMixerToSpeaker, sinks num: " + sinks.length);
            if (sinks.length > 1) {
                continue;
            }
            AudioPortConfig sourceConfig = sources[0];
            ...
            AudioPort sourcePort = sourceConfig.port();
            AudioPort sinkPort = sinkConfig.port();
            Log.d(TAG, "isPatchMixerToSpeaker " + sourcePort + " ====> " + sinkPort);
            if (sourcePort instanceof AudioMixPort && sinkPort instanceof AudioDevicePort) {
                deviceCount++;
                int type = ((AudioDevicePort) sinkPort).type();
        ...
        Log.d(TAG, "isPatchMixerToSpeaker: false " + deviceCount + ", " + deviceEarphoneCount);
        return false;
    }
    ...
    private boolean isPatchContainSpeakerAndEarphone(ArrayList<AudioPatch> patches) {
        ...
        for (AudioPatch patch : patches) {
            AudioPortConfig[] sources = patch.sources();
            AudioPortConfig[] sinks = patch.sinks();
            Log.d(TAG, "isPatchContainSpeakerAndEarphone, sinks num: " + sinks.length);
            ...
            Log.d(TAG, "isPatchContainSpeakerAndEarphone " + sources[0].port() + " ====> "
                    + sinks[0].port() + " + " + sinks[1].port());
        ...
        Log.d(TAG, "isPatchContainSpeakerAndEarphone: ret = " + 
              (hasSpeakerSink && hasEarphoneSink));
    ...
    private boolean isPatchMixerToDeviceRemoved(ArrayList<AudioPatch> patches) {
        ...
        Log.d(TAG, "isPatchMixerToDeviceRemoved: ret = " + noMixerToDevice);
    ...
    private boolean isOutputDeviceChanged(ArrayList<AudioPatch> patches) {
        ...
        for (AudioPatch aPatch : patches) {
            ...
            AudioPort sourcePort = sourceConfig.port();
            AudioPort sinkPort = sinkConfig.port();
            Log.d(TAG, "DEBUG " + sourcePort + " sink: " + sinkPort + " origPort: " + origSinkPort);
            if (sourcePort instanceof AudioMixPort && sinkPort instanceof AudioDevicePort &&
    ...
    private void unregisterSdcardListener() {
    		Log.d(TAG, "unregisterSdcardListener");
    ...
    private void focusChanged(int focusState) {
    		Log.d(TAG, "focusChanged: focusState = " + focusState);
        mIsAudioFocusHeld = false;
        if (mIsNativeScanning || mIsNativeSeeking) {
    ...
    private void updateAudioFocus(int focusState) {
        ...
        switch (focusState) {
            ...
            case AudioManager.AUDIOFOCUS_GAIN:
            	boolean isSpeakerUsed = mIsSpeakerUsed;
    ...
    public void registerFmRadioListener(FmListener callback) {
    		Log.d(TAG, "registerFmRadioListener");
    ...
    private boolean firstPlaying(float frequency) {
    		Log.d(TAG, "firstPlaying, frequency = " + frequency);
        if (mPowerStatus != POWER_UP) {
