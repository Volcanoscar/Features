...
public class SmsPreferenceActivity extends PreferenceActivity implements
    ...
    public boolean onPreferenceChange(Preference preference, Object arg1) {
        ...
        } else if (SMS_INPUT_MODE.equals(key)) {
            if (MmsConfig.isEnableSmsEncodingType()) {
    ...
    //aoran modify for POPLUSVNMS-30 start
    private void  resetSmsQuickTextTemplates()
    {

	final Context runContext = SmsPreferenceActivity.this;
	new Thread(new Runnable() {
			public void run() {
			...
			runContext.getContentResolver().delete(Telephony.MmsSms.CONTENT_URI_QUICKTEXT,
                    ...
                    for (int i = 0; i < default_quick_texts.length;  i++) {
                        ...
                        values.put(_ID,new Integer(i));
                        values.put("text", default_quick_texts[i]);
