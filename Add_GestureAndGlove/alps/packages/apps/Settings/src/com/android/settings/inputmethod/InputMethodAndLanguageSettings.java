...
import com.android.settings.MLTGoodixService;  
...
public class InputMethodAndLanguageSettings extends SettingsPreferenceFragment
	...
	//caoqiaofeng add MTSFEFL-14 20150323 begin
    private static final String KEY_GLOVE_MODE = "glove_mode";
    private SwitchPreference mGloveMode ;   

    public static final String TP_GLOVE_SWITCH_BROADCASE= "tp_glove_switch_broadcase";
	...
	if (mGloveMode != null) {
		int status = Settings.System.getInt(getActivity().getContentResolver(), Settings.System.GLOVE, -1);
		mGloveMode.setChecked(status>0 ? true : false);
	...
	mIntentFilter.addAction(TP_GLOVE_SWITCH_BROADCASE);  
       //caoqiaofeng add MTSFEFL-14 20150323 end
    }
	...
	//caoqiaofeng add MTSFEFL-14 20150323 start
        if(preference.getKey().equals(KEY_GLOVE_MODE)){
        	boolean mBoolean = (Boolean)value ;
        	Log.i("MLT", "mBoolean = " + mBoolean);
        	if(mBoolean){
        		int mEnable = Goodix.enableStylus();
        		if(mEnable > 0 ){
        			Log.i("MLT","enable Goodix funtion success mEnable = " + mEnable);
        			setGoodixSwitchStatus(mBoolean);
        		}else{
        			Log.i("MLT","enable Goodix funtion failed mEnable = " + mEnable);
        		}
        	}else{
        		int mDisable = Goodix.disableStylus();
        		if(mDisable > 0 ){
        			Log.i("MLT","Disable Goodix funtion success mDisable = " + mDisable);
        			setGoodixSwitchStatus(mBoolean);
        		}else{
        			Log.i("MLT","Disable Goodix funtion failed mDisable = " + mDisable);
        		}
        	}

		sendSwitchBroadcase(TP_GLOVE_SWITCH_BROADCASE, "tp_glove_switch_value", mBoolean); 
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(Intent.ACTION_BATTERY_CHANGED) && (mGloveMode != null)) {
			
				int status = intent.getIntExtra("status", 0);
				int plugged = intent.getIntExtra("plugged", 0);
				int mResult = Settings.System.getInt(context.getContentResolver(), Settings.System.GLOVE, -1);
				
				if ((status == BatteryManager.BATTERY_STATUS_CHARGING )|| (plugged != 0)) {
					mGloveMode.setEnabled(false);	
					if ((mResult == MLTGoodixService.ENABLE_STYLUS) && mEnableFlag) {   
						mGloveMode.setChecked(false);	
					...
					if (mDisableFlag) {  
						mGloveMode.setChecked(true);	
						mEnableFlag = true; 
						mDisableFlag = false;
					}
				} 
				
			}else if(action.equals(TP_GLOVE_SWITCH_BROADCASE) && (mGloveMode != null) && (mEnableFlag)){
    ...
    private void setGoodixSwitchStatus(boolean mBoolean){ 
		Settings.System.putInt(getActivity().getContentResolver(), Settings.System.GLOVE, mBoolean ? 1 : 0); 
    }
 
	private void setGustureSwitchStatus(boolean mBoolean){ 
		Settings.System.putInt(getActivity().getContentResolver(), Settings.System.GESTURE, mBoolean ? 1 : 0); 		
    }
