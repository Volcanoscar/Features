...
#ifdef GESTURE_SWITCH_CTRL_HARDWARE
...
static  atomic_t geture_switch_status = ATOMIC_INIT(0);
struct semaphore ft_rw_i2c_sem;
#endif
#ifdef GLOVES_USE_SWITCH
static u8 config_sensor_id =0;//add zhjg
#define GLOVE_SWITCH_OPEN            0x53
#define GLOVE_SWITCH_CLOSE           0x54
#endif
static int tpd_gesture_is_switch_enable(void);
...
*/
	...
	#define KEY_GESTURE_DOWN            61
	#define KEY_GESTURE_S               62
	#define KEY_GESTURE_V               63
	#define KEY_GESTURE_Z               64
	#define KEY_GESTURE_O               65
	#define KEY_GESTURE_W               66
	#define KEY_GESTURE_C               67
	#define KEY_GESTURE_E               68
	#define KEY_GESTURE_M               87
	#define KEY_GESTURE_UP              88
	#define KEY_GESTURE_DOUBLECLICK              89
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    {
    #if GTP_GESTURE_WAKEUP
       if(tpd_gesture_is_switch_enable())
       {
...
static int tpd_misc_open(struct inode *inode, struct file *filp)
{
	return 0;
}
...
*/
static int tpd_misc_release(struct inode *inode, struct file *filp)
{

	return 0;
}
static void tpd_gesture_switch_enable(int enable)
{
	atomic_set(&geture_switch_status,enable);
}
static int tpd_gesture_is_switch_enable(void)
{
	return atomic_read(&geture_switch_status);
}
#ifdef GLOVES_USE_SWITCH
static int tpd_glove_switch_enable(int enable)
{ 
	u8 retry =0;
	int ret = -1;

	u8 cfg_info_group0_gloves_close[] = CTP_CFG_GROUP0_GLOVES_CLOSE;
       u8 cfg_info_group0[] = CTP_CFG_GROUP0;
	u8 cfg_info_group1_gloves_close[] = CTP_CFG_GROUP1_GLOVES_CLOSE;
       u8 cfg_info_group1[] = CTP_CFG_GROUP1;
       int sensor_id = config_sensor_id*2+enable;
       u8 *send_cfg_buf[] = {
                                                  cfg_info_group0_gloves_close, 
                                                  cfg_info_group0, 
      							 cfg_info_group1_gloves_close, 
      							 cfg_info_group1
                      			};

        u8 cfg_info_len[] = {
	                   CFG_GROUP_LEN(cfg_info_group0_gloves_close), 
                          CFG_GROUP_LEN(cfg_info_group0),
                          CFG_GROUP_LEN(cfg_info_group1_gloves_close),
                          CFG_GROUP_LEN(cfg_info_group1)
                          };
        u8 config_buf[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
    = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff}; 


        memset(&config_buf[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
        memcpy(&config_buf[GTP_ADDR_LENGTH], send_cfg_buf[sensor_id], cfg_info_len[sensor_id]);

	for(retry=0;retry<5;retry++)
	{
	        if(send_cfg_buf[sensor_id][0] != 0xff)
	        {
	        	ret = gtp_i2c_write(i2c_client_point,&config_buf[0],GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
		 }
		if(ret >0)
		{
			break;
...
#endif
static int tpd_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int ret = 0;
//	struct ft_rw_i2c_dev *ftdev = filp->private_data;
//	ftdev = filp->private_data;

	GTP_INFO(" tpd_unlocked_ioctl cmd=0x%x\n",cmd);
	...
	case GESTURE_SWITCH_OPEN:
		tpd_gesture_switch_enable(1);
		GTP_INFO("tpd_unlocked_ioctl gesture switch = GESTURE_SWITCH_OPEN...\n");
		break;

	case GESTURE_SWITCH_CLOSE:
		tpd_gesture_switch_enable(0);
		GTP_INFO("tpd_unlocked_ioctl gesture switch = GESTURE_SWITCH_CLOSE...\n");
		break;
#ifdef GLOVES_USE_SWITCH
	case GLOVE_SWITCH_OPEN:
		ret = tpd_glove_switch_enable(1);
		GTP_INFO("ft_rw_iic_drv_ioctl glove switch = GLOVE_SWITCH_OPEN...\n");
		break;
	case GLOVE_SWITCH_CLOSE:
		ret = tpd_glove_switch_enable(0);
		GTP_INFO("ft_rw_iic_drv_ioctl glove switch = GLOVE_SWITCH_CLOSE...\n");
		break;
#endif
	default:
		ret =  -ENOTTY;
...
static int tpd_gesture_is_switch_enable(void)
{
      #if GTP_GESTURE_WAKEUP //open  GTP_GESTURE_WAKEUP ==1 && !defined(GESTURE_SWITCH_CTRL_HARDWARE)
		return 1;
	#else
		return 0;
	#endif
...
//#ifdef VELOCITY_CUSTOM
	...
	input_set_capability(tpd->dev, EV_KEY, KEY_GESTURE_DOUBLECLICK);//add zhjg 20150807
    #else
       input_set_capability(tpd->dev, EV_KEY, KEY_POWER);
    #endif
#endif
...
#ifdef GESTURE_SHOW_ANIMATION
static void gt9xx_tpd_gesture_report_key(u8 key)
{

	if (key != KEY_GESTURE_NONE)
	{
...
static void gt9xx_tpd_gesture_report(int gesture_id)
{
	GTP_INFO("tpd_gesture_report gesture_id = 0x%x...\n", gesture_id);

	switch (gesture_id)
	{
	case GESTURE_DOUBLECLICK:
		gt9xx_tpd_gesture_report_key(KEY_GESTURE_DOUBLECLICK);//add zhjg 20150807
		break;
	case GESTURE_LEFT:
	case GESTURE_RIGHT:
		//tpd_gesture_report_key(KEY_POWER, KEY_GESTURE_NONE);
		break;

	case GESTURE_UP:
		gt9xx_tpd_gesture_report_key(KEY_GESTURE_UP);
		break;

	case GESTURE_DOWN:
		gt9xx_tpd_gesture_report_key(KEY_GESTURE_DOWN);
		break;

	case GESTURE_O:
		gt9xx_tpd_gesture_report_key(KEY_GESTURE_O);
		break;

	case GESTURE_W:
		gt9xx_tpd_gesture_report_key(KEY_GESTURE_W);
		break;

	case GESTURE_M:
		gt9xx_tpd_gesture_report_key(KEY_GESTURE_M);
		break;

	case GESTURE_E:
		gt9xx_tpd_gesture_report_key(KEY_GESTURE_E);
		break;

	case GESTURE_C:
		gt9xx_tpd_gesture_report_key(KEY_GESTURE_C);
		break;

	case GESTURE_S:
		gt9xx_tpd_gesture_report_key(KEY_GESTURE_S);
		break;

	case GESTURE_V:
		gt9xx_tpd_gesture_report_key(KEY_GESTURE_V);
		break;

	case GESTURE_Z:
		gt9xx_tpd_gesture_report_key(KEY_GESTURE_Z);
		break;
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
    ...
    if(tpd_gesture_is_switch_enable())
    {
        if (DOZE_ENABLED == doze_status)
        {
            ...
            {     
            #ifdef GESTURE_SHOW_ANIMATION
                    gt9xx_tpd_gesture_report(doze_buf[2]);
...
static s8 gtp_wakeup_sleep(struct i2c_client *client)
{
    ...
    while (retry++ < 10)
    {
	  ...
	  else
	  {
        ...
        #if (!GTP_GESTURE_WAKEUP  || defined(GESTURE_SWITCH_CTRL_HARDWARE))
            if(!tpd_gesture_is_switch_enable())
            {
                gtp_int_sync(25);
            #if GTP_ESD_PROTECT
