...
public class PhoneStatusBarPolicy {
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            /// @}
	     //caoqiaofeng add MTSFEFL-14 20150323 start	     
		else if (action.equals(TP_GLOVE_SWITCH_BROADCASE)) {
			updateGlove(intent);  
		}
		else if (action.equals(TP_GESTURE_SWITCH_BROADCASE)) {
			updateGesture(intent);
		}
	     //caoqiaofeng add MTSFEFL-14 20150323  end		
        }
    };
    ...
    public PhoneStatusBarPolicy(Context context, CastController cast, HotspotController hotspot) {
        ...
        /// @}
        //caoqiaofeng add MTSFEFL-14 20150323  start
	 filter.addAction(TP_GLOVE_SWITCH_BROADCASE);
	 filter.addAction(TP_GESTURE_SWITCH_BROADCASE);
	 //caoqiaofeng add MTSFEFL-14 20150323  end	
        ...
        /// @}
        //caoqiaofeng add MTSFEFL-14 20150323  start
	 mService.setIcon("glove", R.drawable.stat_sys_glove, 0, null);  
	 mService.setIconVisibility("glove", false);

	 mService.setIcon("gesture", R.drawable.stat_sys_gesture, 0, null);  
	 mService.setIconVisibility("gesture", false);
        //caoqiaofeng add MTSFEFL-14 20150323 end
    }
    ...
    /// M: [Multi-User] register Alarm intent by user @}
   //caoqiaofeng add MTSFEFL-14 20150323  start  
    private final void updateGlove(Intent intent) {
        int state = intent.getIntExtra("tp_glove_switch_value", -1);
   
        //Xlog.d(TAG, "updateGlove, state=" + state );
        if (state == -1) {
            return;
        }
        if (state == 1) {            
                mService.setIcon("glove", R.drawable.stat_sys_glove, 0, null);
                mService.setIconVisibility("glove", true);
        } else {
            mService.setIconVisibility("glove", false);
        }
    }

    private final void updateGesture(Intent intent) {
        int state = intent.getIntExtra("tp_gesture_switch_value", -1);
   
        //Xlog.d(TAG, "updateGesture, state=" + state );
        if (state == -1) {
            return;
        }
        if (state == 1) {            
                mService.setIcon("gesture", R.drawable.stat_sys_gesture, 0, null);
               mService.setIconVisibility("gesture", true);
        } else {
            mService.setIconVisibility("gesture", false);
        }
    } 
