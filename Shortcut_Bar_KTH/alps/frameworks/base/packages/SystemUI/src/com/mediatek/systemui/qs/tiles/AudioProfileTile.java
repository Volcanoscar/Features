...
public class AudioProfileTile extends QSTile<QSTile.BooleanState> {
	...
	private static final String MTK_AUDIOPROFILE_GENEEAL = "mtk_audioprofile_general";
	private static final String MTK_AUDIOPROFILE_SILENT = "mtk_audioprofile_silent";
	private static final String MTK_AUDIOPROFILE_MEETING = "mtk_audioprofile_meeting";
	private static final String MTK_AUDIOPROFILE_OUTDOOR = "mtk_audioprofile_outdoor";
	...
	private static int stateMeeting = 1;
	private static final String TAG_KTH = "ShortcutBar_kth";

	private BroadcastReceiver controlReceiver = new BroadcastReceiver() {
		@Override
		public void onReceive(Context context, Intent intent) {
			Log.i(TAG_KTH, "AudioProfileTile audioprofile received = "+ intent.getAction());
			if (intent.getAction().equals(CONTROLACTION_AUDIOPROFILE)) {							
				if (ENABLE_AUDIO_PROFILE) {
					String key = null;
					ContentResolver resolver = mContext.getContentResolver();
					int mode_audioprofile_last = Settings.Global.getInt(resolver, Settings.Global.AUDIOPROFILE_LAST_STATE, 0);
					...
					if(mode_audioprofile_now == 2){
						switch(mode_audioprofile_last){
						case 0:
							key = MTK_AUDIOPROFILE_GENEEAL;
							break;
						case 1:
							key = MTK_AUDIOPROFILE_SILENT;
							break;
						case 3:
							key = MTK_AUDIOPROFILE_OUTDOOR;
							break;
						default:
							key = MTK_AUDIOPROFILE_GENEEAL;
							break;
						...
						key = MTK_AUDIOPROFILE_MEETING;
					}
			        Log.i(TAG_KTH, "AudioProfileTile stateMeeting is ="+stateMeeting+" key = " + key);
    ...
    public AudioProfileTile(Host host) {
		...
		// kth add  LFZSF-10 for shorcut bar test at 20150820 start
		IntentFilter control_filter = new IntentFilter();
		control_filter.addAction(CONTROLACTION_AUDIOPROFILE);
    ...
    private void updateAudioProfile(String key) {
        ...
        if (ENABLE_AUDIO_PROFILE) {
			// kth add for  LFZSF-10 shortcut bar test at 20150820 start
			if(key.equals(MTK_AUDIOPROFILE_SILENT)){
				Settings.Global.putInt(mContext.getContentResolver(),
                			Settings.Global.AUDIOPROFILE_LAST_STATE, 1);
				Settings.Global.putInt(mContext.getContentResolver(),
                			Settings.Global.AUDIOPROFILE_NOW_STATE, 1);
			}else if(key.equals(MTK_AUDIOPROFILE_MEETING)){
				Settings.Global.putInt(mContext.getContentResolver(),
                			Settings.Global.AUDIOPROFILE_NOW_STATE, 2);
				Vibrator vib = (Vibrator) mContext.getSystemService(Service.VIBRATOR_SERVICE);
				vib.vibrate(150);
			}else if(key.equals(MTK_AUDIOPROFILE_OUTDOOR)){
				Settings.Global.putInt(mContext.getContentResolver(),
                			Settings.Global.AUDIOPROFILE_LAST_STATE, 3);
				Settings.Global.putInt(mContext.getContentResolver(),
                			Settings.Global.AUDIOPROFILE_NOW_STATE, 3);
			}else if(key.equals(MTK_AUDIOPROFILE_GENEEAL)){
				Settings.Global.putInt(mContext.getContentResolver(),
                			Settings.Global.AUDIOPROFILE_LAST_STATE, 0);
				Settings.Global.putInt(mContext.getContentResolver(),
                			Settings.Global.AUDIOPROFILE_NOW_STATE, 0);
			}
			// kth add for  LFZSF-10 shortcut bar test at 20150820 end
        ...
        public void onProfileChanged(String profileKey) {
            if (ENABLE_AUDIO_PROFILE) {
                if (profileKey != null) {
                    ...
                    if (senario != null) {
						 Log.i("TEST_KONG","mAudioProfileListenr senario != null");
                        updateProfileView(senario);
                    }
                }
            }
    ...
    private void updateProfileView(Scenario scenario) {
		...
		updateIntent.setAction(CONTROLACTION_AUDIOPROFILE_UPDATE_STATE); // kth add shortcut bar 20150820
    ...
    private void loadDisabledProfileResouceForAll() {
		...
		updateIntent.putExtra("msg", 0);//always make the audio profile icon disabled first include the custom meeting icon.-add kth  LFZSF-10 20150820
    ...
    private void loadEnabledProfileResource(Scenario scenario) {
        ...
        int audioState;
		Log.i(TAG_KTH, "AudioProfileTile loadEnabledProfileResource is " + scenario);
		stateMeeting = 0;
        switch (scenario) {
        case GENERAL:
        ...
        case MEETING:
			stateMeeting = 1;
			updateIntent.putExtra("msg", 1);
			mContext.sendBroadcast(updateIntent);// kth add  LFZSF-10 for update shortcut UI state if status bar and shortcut bar both appear 20150820 
