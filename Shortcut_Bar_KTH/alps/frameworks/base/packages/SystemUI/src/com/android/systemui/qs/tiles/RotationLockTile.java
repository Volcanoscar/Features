...
public class RotationLockTile extends QSTile<QSTile.BooleanState> {
	...
	private BroadcastReceiver controlReceiver = new BroadcastReceiver() {
		...
		Log.i(TAG_KTH, "RotationLockTile rotation received = "+ intent.getAction());
			if (intent.getAction().equals(CONTROLACTION_ROTATION)) {
				click();
				refreshState();			
			}
		}
    ...
    public RotationLockTile(Host host) {
		...
		control_filter.addAction(CONTROLACTION_ROTATION);
		mContext.registerReceiver(controlReceiver, control_filter);
		// kth add LFZSF-10 for shorcut bar test at 20150820 end
    }
    ...
    protected void handleUpdateState(BooleanState state, Object arg) {
		...
		// kth add for LFZSF-10 shorcut bar test at 20150820 start
		updateIntent.setAction(CONTROLACTION_ROTATION_UPDATE_STATE);
