...
public class AirplaneModeTile extends QSTile<QSTile.BooleanState> {
	...
	private BroadcastReceiver controlReceiver = new BroadcastReceiver() {
		...
		Log.i(TAG_KTH, "AirplaneModeTile airplane received = "+ intent.getAction());
			if (intent.getAction().equals(CONTROLACTION_AIRPLANE)) {			
				click();
				refreshState();
			}
		}
    ...
    public AirplaneModeTile(Host host) {
		...
		control_filter.addAction(CONTROLACTION_AIRPLANE);
    ...
    protected void handleUpdateState(BooleanState state, Object arg) {
        ...
        if (airplaneMode) {
			TILE_STATE = 1; // kth add for  LFZSF-10 shorcut bar test at 20150820
			state.icon = mEnable;
            state.contentDescription =  mContext.getString(
                    R.string.accessibility_quick_settings_airplane_on);
        } else {
        	TILE_STATE = 0; // kth add for  LFZSF-10 shorcut bar test at 20150820
            state.icon = mDisable;
            state.contentDescription =  mContext.getString(
                    R.string.accessibility_quick_settings_airplane_off);
        }
		// kth add for  LFZSF-10 shorcut bar test at 20150820 start
		updateIntent.setAction(CONTROLACTION_AIRPLANE_UPDATE_STATE);
