...
public class WifiTile extends QSTile<QSTile.SignalState> {
	...
	private BroadcastReceiver controlReceiver = new BroadcastReceiver() {
		...
		Log.i(TAG_KTH, "WifiTile received = "+ intent.getAction());
			if (intent.getAction().equals(CONTROLACTION_WIFI)) {
				Log.i(TAG_KTH, "WifiTile receive broadcast_wifi==1");		
				click();
				setListening(true);
			}
			else if (intent.getAction().equals(CONTROLACTION_WIFI_DETAIL)) {
    ...
    public QSTileView createTileView(Context context) {
		...
		control_filter.addAction(CONTROLACTION_WIFI);
		control_filter.addAction(CONTROLACTION_WIFI_DETAIL);
		context.registerReceiver(controlReceiver, control_filter);
    ...
    protected void handleUpdateState(SignalState state, Object arg) {
		...
		Log.i(TAG_KTH, "WifiTile handleUpdateState wifi enable--"+state.enabled);
        if (!state.enabled) {
			...
			TILE_STATE = 0; //kth add for shortcut bar 2010820
        ...
        if (state.connected) {
			...
			TILE_STATE = 3; //kth add 2010820
        }
        state.dualLabelContentDescription = wifiName;
		// kth add for LFZSF-10 shortcut bar at 2010820 start
		updateIntent.setAction(CONTROLACTION_WIFI_UPDATE_STATE);		
