...
//caoqiaofeng add MTSFEFL-14 20150323 start
import android.content.SharedPreferences;
import android.content.pm.PackageManager.NameNotFoundException;
import com.android.internal.widget.LockPatternUtils;
//caoqiaofeng add MTSFEFL-14 20150323 end


//aoran add LFZSF-2,start.
import android.hardware.Sensor;
import android.hardware.SensorEvent;
import android.hardware.SensorEventListener;
import android.hardware.SensorManager;
import android.content.ContentResolver;
//aoran add LFZSF-2,end.

import android.os.AsyncTask;

//kth add for LFZSF-10 shortcut bar at 20150820 start
import android.content.pm.PackageManager.NameNotFoundException;
...
// kth add for LFZSF-10 shortcut bar at 20150820 end
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    static final String TAG_KTH = "ShortcutBar_kth";// kth add for debug shortcut bar at 20150820
    ...
    //private boolean mIsFactoryTestShutDown = false;
    //pss
    /**
    ...
    private final static String UPDATE_SHORTCUT_ACTION = "android.intent.action.UPDATE_SHORTCUT_ACTION";
    final static String HOME_DOWN_ACTION = "android.intent.action.HOME_DOWN_ACTION";
    final static String ROTATION_CHANGE_ACTION = "android.intent.action.ROTATION_CHANGE_ACTION_SHORTCUT";
    private BroadcastReceiver updateshorcutReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(final Context context, final Intent intent) {

            Log.i("TEST_KONG", "PhoneWindowManager received " + intent.getAction());
            mHandler.post(new Runnable() {

                @Override
                public void run() {
                    mHandler.postDelayed(mHomeLongPress,0);
                }
            });
        }
    };
	...
	*/
    ...
    //caoqiaofeng add MTSFEFL-14 20150323 start	
    public static final String TP_GESTURE_BROADCASE = "android.intent.guest_broadcase";
    static final int GESTURE_C_MODE = 139;
    static final int GESTURE_E_MODE = 140;
    static final int GESTURE_M_MODE = 141;
    static final int GESTURE_UP_MODE = 142;
    static final int GESTURE_DOWN_MODE = 133;
    static final int GESTURE_S_MODE = 134;
    static final int GESTURE_V_MODE = 135;
    static final int GESTURE_Z_MODE = 136;
    static final int GESTURE_O_MODE = 137;
    static final int GESTURE_W_MODE = 138;
    ///aoran add "double click" for LFZSF-2,start.
    static final int GESTURE_DOUBLE_CLICK_MODE = 217;
    ///aoran add "double click" for LFZSF-2,end.

    //aoran add LFZSF-2,start.
    private long mTimeGestrueOK;
    private long mTimeVolPressed;
    private boolean mLauncherCamera = false;
    private boolean mLauncherPhone = false;
    ...
    public static final String PHONE_QUICK_CAMERA_ACTION = "android.intent.action.ACTION_QUICK_CAMERA";
    public static final String PHONE_QUICK_PHONE_ACTION = "android.intent.action.ACTION_PHONE_QUICK_PHONE";
    //aoran add LFZSF-2,end.

    private boolean isSlideUnlock = true;
    //caoqiaofeng add MTSFEFL-14 20150323 end
    ...
    private void interceptPowerKeyUp(KeyEvent event, boolean interactive, boolean canceled, int result) { // chb modify for factorytest
        ...
        if (!handled) {
            ...
            // chb modify for factorytest begin 
            // No other actions.  Handle it immediately.
            if(!((result & ACTION_PASS_TO_USER) == ACTION_PASS_TO_USER)){
                powerPress(eventTime, interactive, mPowerKeyPressCounter);
            }
            // chb modify for factorytest end 
        }
    ...
    // kth add for LFZSF-10  shorcut bar test at 20150820 start
    private final Runnable mHomeLongPress = new Runnable() {

        @Override
        public void run() {
            // TODO Auto-generated method stub
            Slog.i(TAG_KTH,"mHomeLongPress");
            if(shortcutDialog==null){
                Log.i(TAG_KTH,"shortcutDialog==null");
                shortcutDialog = new ShortcutDialog(mContext);
            }
            if(!shortcutDialog.isShowing()){
                Log.i(TAG_KTH,"shortcutDialog is not showing");
                shortcutDialog.show();
            }
        }
    };
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        // kth add for LFZSF-10  shortcut bar at 20150820 start
        control_filter.addAction(UPDATE_SHORTCUT_ACTION);
        mContext.registerReceiver(updateshorcutReceiver, control_filter);
        // kth add for LFZSF-10  shortcut bar at 20150820 end
        ...
        ///PSS: register for power-off FactoryTest shutDown @{
		/*IntentFilter poweroffFactoryTestFilter = new IntentFilter();
		poweroffFactoryTestFilter.addAction(FACTORYTEST_NORMAL_POWERKEY_SHUTDOWN_ACTION);
		poweroffFactoryTestFilter.addAction(FACTORYTEST_NORMAL_POWERKEY_RENEW_ACTION);
		context.registerReceiver(mPoweroffFactoryTestReceiver, poweroffFactoryTestFilter);
		*/
        ///@}
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            ...
            // add begin for test shuyong   
            Log.i("pss","className : = "+getFactoryTestClassName());
            ActivityManager activityManager = (ActivityManager) mContext
                    .getSystemService(Context.ACTIVITY_SERVICE);
            try {
                if (activityManager.getRunningTasks(1).get(0).topActivity
                        .getClassName().equals("com.mlt.factorytest.ItemTestActivity") && getFactoryTestClassName()
                        .equals("com.mlt.factorytest.item.KeyAndMotor")) {
                    return 0;
                }
            } catch (NullPointerException e) {
                Slog.d(TAG,"detect power key exception!");
            }
            //add end
            if (!down) {
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        //  PSS: FactoryTest s551 2015 03 02
        ActivityManager activityManager = (ActivityManager) mContext
                .getSystemService(Context.ACTIVITY_SERVICE);
			/*try {
				if (activityManager.getRunningTasks(1).get(0).topActivity
						.getClassName().equals("com.mlt.factorytest.ItemTestActivity")
						&& KeyEvent.KEYCODE_POWER == event.getKeyCode() && mIsFactoryTestShutDown) {
					Intent intent = new Intent();
					intent.setAction(FACTORYTEST_NORMAL_POWERKEY_DISABLE_ACTION);
			...
			try {
				if (activityManager.getRunningTasks(1).get(0).topActivity
						.getClassName().equals("com.mlt.factorytest.ItemTestActivity")
						&& KeyEvent.KEYCODE_HOME == event.getKeyCode() && mIsFactoryTestShutDown) {
					Intent intent = new Intent();
					intent.setAction(FACTORYTEST_HOME_DISABLE_ACTION);
					mContext.sendBroadcast(intent);
					return 0;
				}
			} catch (NullPointerException e) {
			} */
        // PSS: FactoryTest S511 2015 03 02
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE: {
                //aoran add LFZSF-2,start.
                boolean bQuickLaunchCameraOn = 1==Settings.Secure.getInt(cr,
                        Settings.System.DEF_SMART_ACTION,
                        Settings.Secure.DEF_QUICK_LAUNCH_CAMERA, 0) ;
                boolean bQuickLaunchPhoneOn = 1==Settings.Secure.getInt(cr,
                        Settings.System.DEF_SMART_ACTION,
                        Settings.Secure.DEF_QUICK_LAUNCH_PHONE, 0) ;
                mTimeVolPressed = System.currentTimeMillis();
                boolean isQuickGestureOn = (mTimeVolPressed - mTimeGestrueOK)< 2000;
                Log.d("aoran","mTimeVolPressed="+mTimeVolPressed);
                Log.d("aoran","mTimeGestrueOK="+mTimeGestrueOK);
                Log.d("aoran","isQuickGestureOn="+isQuickGestureOn);
                Log.d("aoran","bQuickLaunchCameraOn = "+bQuickLaunchCameraOn+" bQuickLaunchPhoneOn = "+bQuickLaunchPhoneOn);
                if(mLauncherCamera || mLauncherPhone)
                {
                    if((bQuickLaunchCameraOn && mLauncherCamera) && isQuickGestureOn)
                    {
                        broadcastQuickCamera();
                        if(sensorEnabled)
                        {
                            enableSensors(false);
                            Log.d("aoran","forQueue C enableSensors(false)");
                        }
                        vibrate();
                        wakeUpScreen();
                    }else if((bQuickLaunchPhoneOn && mLauncherPhone) && isQuickGestureOn)
                    {
                        wakeUpScreen();
                        broadcastQuickPhone();
                        vibrate();
                        ...
                        {
                            enableSensors(false);
                            Log.d("aoran","forQueue P enableSensors(false)");
                        }
                    }
                    break;
                }
            ...
            case KeyEvent.KEYCODE_POWER: {
                ...
                try {
                    if (activityManager.getRunningTasks(1).get(0).topActivity
                        ...
                        result |= ACTION_PASS_TO_USER;
                        Slog.d(TAG,"detect power key in special package, pass to user!");
                    }
                } catch (NullPointerException e) {
                    Slog.d(TAG,"detect power key exception!");
                }
                ...
                if (down) {
                    ...
                    //if(!((result & ACTION_PASS_TO_USER) == ACTION_PASS_TO_USER)){
                    interceptPowerKeyUp(event, interactive, canceled,result); // chb modify for factorytest
                    //}
                    // kth add for LFZSF-10  shorcut bar test at 20150820 start
                    if(shortcutDialog!=null && shortcutDialog.isShowing()){
                        shortcutDialog.dismiss();
            ...
            ///aoran add "double click" for LFZSF-2,start.
            case GESTURE_DOUBLE_CLICK_MODE:
                ///aoran add "double click" for LFZSF-2,end.
                //caoqiaofeng add MTSFEFL-14 20150323 start
            case GESTURE_C_MODE:
            case GESTURE_E_MODE:
            ...
            case GESTURE_V_MODE:
            case GESTURE_Z_MODE:
            case GESTURE_DOWN_MODE:
            case GESTURE_UP_MODE:	{
                if (down) {    //cqf add SFZZANPCLL-102 20150603               
    ...
    public String getFactoryTestClassName(){
        Context contextFactoryTest= null;
        try {
            contextFactoryTest =  mContext.createPackageContext("com.mlt.factorytest", 0x2);
            SharedPreferences share = contextFactoryTest.getSharedPreferences("FactoryTestSharePreference", 4);
            Log.i("pss","share = "+share);
            return share.getString("classname", "factorytest");
    ...
    //pss add FactoryTest for keyandmorot 20150714 end
    ...
    //caoqiaofeng add MTSFEFL-14 20150323 start
    private void startSmartGesture(int keyCode){

        Log.d(TAG, "malata startSmartGesture keyCode="+keyCode);
        Context contextSetting = null;
        try {
            contextSetting = mContext.createPackageContext("com.android.settings", 0x2);
        } catch (NameNotFoundException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
        ...
        if(keyCode == GESTURE_C_MODE ){
            sGesturesKey = "gestures_settings_c";
        }else if(keyCode == GESTURE_E_MODE ){
            sGesturesKey = "gestures_settings_e";
        }else if(keyCode == GESTURE_M_MODE ){
            sGesturesKey = "gestures_settings_m";
        }else if(keyCode == GESTURE_UP_MODE ){
            sGesturesKey = "gestures_settings_up";
        }else if(keyCode == GESTURE_O_MODE ){
            sGesturesKey = "gestures_settings_o";
        }else if(keyCode == GESTURE_W_MODE ){
            sGesturesKey = "gestures_settings_w";
        }else if(keyCode == GESTURE_DOWN_MODE ){
            sGesturesKey = "gestures_settings_down";
        }else if(keyCode == GESTURE_S_MODE ){
            sGesturesKey = "gestures_settings_s";
        }else if(keyCode == GESTURE_V_MODE ){
            sGesturesKey = "gestures_settings_v";
        }else if(keyCode == GESTURE_Z_MODE ){
            sGesturesKey = "gestures_settings_z";
            ///aoran add "double click" for LFZSF-2,start.
        }else if(keyCode == GESTURE_DOUBLE_CLICK_MODE){
            sGesturesKey = "gestures_settings_doubleclick";
            ///aoran add "double click" for LFZSF-2,end.
        }
        ...
        Log.d(TAG, "malata startSmartGesture gestrueStatus="+gestrueStatus);
        int status = Settings.System.getInt(mContext.getContentResolver(), Settings.System.GESTURE, -1);

        if(gestrueStatus && (status>0)){
            ///aoran add "double click" for LFZSF-2 @{
            if(keyCode == GESTURE_DOUBLE_CLICK_MODE)
            {
                wakeUpScreen();
            }else{///@}
                Intent mIntent = new Intent();
                ComponentName localComponentName = new ComponentName("com.malatamobile.smartgesture", "com.malatamobile.smartgesture.GestureHandlerActivity");
                mIntent.setComponent(localComponentName);
                mIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                Bundle bundle = new Bundle();
                bundle.putInt("gesture_action_type", keyCode);
                bundle.putString("packageName", packageName);
                bundle.putString("activityName", activityName);
                mIntent.putExtras(bundle);
                Log.d(TAG, "malata startSmartGesture isSlideUnlock="+isSlideUnlock);
                if(isSlideUnlock){
                    mKeyguardDelegate.keyguardDone(true,true);
                    mContext.startActivity(mIntent);
                }else{
                    startDefaultActivity(packageName, activityName);
                }
            }
        }
    }

    private void startDefaultActivity(String packageName, String activityName) {
        wakeUpScreen();
    ...
    private void wakeUpScreen() {
        ...
        PowerManager.WakeLock mWakelock = pm.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK|PowerManager.ACQUIRE_CAUSES_WAKEUP, "wakeUpScreen");
    ...
    public void screenTurnedOff() {
        ...
        if(bQuickLaunchCameraOn || bQuickLaunchPhoneOn)
        {
            enableSensors(true);
            sensorEnabled = true;
            Log.d("aoran","screenTurnedOff enableSensors(true)");
        }
        //aoran add LFZSF-2,end.
    }
    ...
    public void screenTurningOn(final ScreenOnListener screenOnListener) {
        ...
        {
            enableSensors(false);
            Log.d("aoran","screenTurningOn enableSensors(false)");
        }
        //aoran add LFZSF-2,end.
    ...
    //aoran add LFZSF-2,start.
    private void enableSensors(boolean bEnable) {
        if (null == mSensorManager) {
            //Log.e(TAG, "Jiangde OMG null p!!"); 
            return;
        }

        Log.d("aoran","enableSonsors-----"+bEnable);
    ...
    private void broadcastQuickCamera() {
        Log.v("aoran", "broadcastQuickCamera");
        Intent broadcast = new Intent(PHONE_QUICK_CAMERA_ACTION);
        broadcast.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT
                | Intent.FLAG_RECEIVER_REPLACE_PENDING);
        mContext.sendBroadcast(broadcast);
    }

    private void broadcastQuickPhone() {
        Log.v(TAG, "broadcastQuickPhone");
        Intent broadcast = new Intent(PHONE_QUICK_PHONE_ACTION);
        broadcast.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT
                | Intent.FLAG_RECEIVER_REPLACE_PENDING);
        mContext.sendBroadcast(broadcast);
    }

    private class AccelerometerSilentListener implements SensorEventListener{ // Jiangde Silent

        private float z = 0;
        private float az = 0;
        private boolean isFirstLaunchCamera = true;
        private boolean isFirstLaunchPhone = true;
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            if ((az > -3.5 && az < 5 )
                    ...
                    && (ay > -3.5 && ay < 5 )
						/*&& faceUp*/) {
                //faceUp = false;
                mLauncherCamera = true;
                if(isFirstLaunchCamera){
                    Log.d("aoran","-------FirstLaunchCamera-----------");
                    mTimeGestrueOK = System.currentTimeMillis();
                    Log.d("aoran","C mTimeGestrueOK="+mTimeGestrueOK);
                    isFirstLaunchCamera = false;
                }
                //Log.d("aoran","mLauncherCamera = true;");
            }
            else{
                mLauncherCamera= false;
                isFirstLaunchCamera = true;
                //Log.d("aoran","mLauncherCamera = false;");
            }
            //z = az;
            ...
            if ((az > -3.5 && az < 8)
                ...
                //faceUp = false;
                mLauncherPhone= true;
                if(isFirstLaunchPhone){
                    Log.d("aoran","-------FirstLaunchPhone-----------");
                    mTimeGestrueOK = System.currentTimeMillis();
                    Log.d("aoran","P mTimeGestrueOK="+mTimeGestrueOK);
                    isFirstLaunchPhone = false;
                }
                //Log.d("aoran","mLauncherPhone = true;");
            }
            else{
                mLauncherPhone= false;
                isFirstLaunchPhone = true;
                //Log.d("aoran","mLauncherPhone = false;");
            }
            //z = az;
