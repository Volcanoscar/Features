...
/// @}
    ...
    private void showSavingProgress(String albumName) {
        ...
        /// M: [FEATURE.MODIFY] Make filtershow be consistent with gallery @{
        /*String progressText;
        if (albumName == null) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        /// M: [DEBUG.ADD] No response for onClick event, while in loading view @{
        if (mLoadingVisible) {
...
/// M: [BEHAVIOR.ADD] @{
    ...
    public void saveImage() {
        if (mImageShow.hasModifications()) {
			...
			// kth delete it for POPLUSVNMS-75 20151222 start
            /// M: [BUG.ADD] disable backpress when saving image @{
            // old value: showSavingProgress(mAlbumNameForSaving);
            /// @}
			// kth delete it for POPLUSVNMS-75 20151222 end
            ...
            /// @}
			// kth add it for POPLUSVNMS-75 20151222 start
			showSavingProgress(albumName);
			File dest = SaveImage.getNewFile(this, getSelectedImageUri());
			SaveImage.saveImageAsACopy(this, dest);
			return;
			// kth add it for POPLUSVNMS-75 20151222 end
            //mImageShow.saveImage(this, null);// kth delete it for POPLUSVNMS-75 20151222 start
        } else {
            done();
        }
    }
