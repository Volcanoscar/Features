package com.mlt.filemanager.search;
...
import com.example.testsearch.SearchAutoAdapter;
...
/** 
* @ClassName: MltFileManagerSearchActivity 
* @Description: 文件搜索类
* @author:   chehongbin
* Copyright (c) 2015 MALATA,All Rights Reserved.
*/
public class MltFileManagerSearchActivity extends AbsBaseActivity {
    private static final String TAG = "MltFileManagerSearchActivity_TAG";
    private TextView mResultView = null;
    private String mSearchPath = null;
    private SearchView mSearchView = null;
    private MenuItem mSearchItem;
    private long mTotal = 0; //搜索到的个数
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            Log.d(TAG, "mSearchPath1:"+mSearchPath);
        } else {
            mSearchPath = getIntent().getStringExtra(CURRENT_PATH);
            Log.d(TAG, "else mSearchPath1:"+mSearchPath); // 传递进来的路径
        }
    }
    ...
    protected void onSaveInstanceState(Bundle outState) {
        if (mSearchText != null) {
            ...
            Log.d(TAG, "SEARCH_TEXT1:"+SEARCH_TEXT+"; mSearchText:"+mSearchText);
            outState.putLong(SEARCH_TOTAL, mTotal);
            Log.d(TAG, "SEARCH_TOTAL2:"+SEARCH_TOTAL+"; mTotal:"+mTotal);
        }
        ...
        Log.d(TAG, "CURRENT_PATH:"+CURRENT_PATH+"; mSearchPath:"+mSearchPath);
    ...
    private void handleIntent(Intent intent) {
        ...
        if (Intent.ACTION_VIEW.equals(intent.getAction())) { // 点击搜索提示列表 打开文件
            String path = null;
            if (intent.getData() != null) {
                path = intent.getData().toString();
                Log.d(TAG, "Intent1:"+intent.getAction());
                Log.d(TAG, "intent1.getData():"+path);
            }
            ...
            Log.d(TAG, "path:"+path);
        } else if (Intent.ACTION_SEARCH.equals(intent.getAction())) { //搜索显示
            // handles a search query
        	Log.d(TAG, "Intent2:"+intent.getAction());
            String query = intent.getStringExtra(SearchManager.QUERY);
            Log.d(TAG, "SearchManager.QUERY:"+SearchManager.QUERY);
        ...
        if (query != null && !query.isEmpty()) {
            if (mService != null) {
                mService.search(this.getClass().getName(), query, mSearchPath, new SearchListener(
                        query));
                Log.d(TAG, "mSearchPath_query:"+mSearchPath);
                if (mSearchView != null) {
    ...
    //chb add 
    @Override
    public boolean onSearchRequested() { // //打开浮动搜索框（第一个参数默认添加到搜索框的值） 
    ...
    protected class SearchListener implements FileManagerService.OperationEventListener {
        ...
        @Override
        public void onTaskResult(int result) { //更新显示结果
        ...
        public void onTaskProgress(ProgressInfo progressInfo) {
            if (!progressInfo.isFailInfo()) {
                if (mResultView != null && !mIsResultSet) {
                    ...
                    mResultView.setVisibility(View.VISIBLE); //显示搜索结果的数量
                    mResultView.setText(getResources().getString(
                            R.string.search_result, mSearchText, mTotal));
                    Log.d(TAG, "mResultView:"+mResultView+" ;search_result"+R.string.search_result+" ;mTotal: "+mTotal);
                    mIsResultSet = true;
                }
	...
	OnQueryTextListener searchQueryTextListener = new OnQueryTextListener() {
		
		@Override
		public boolean onQueryTextSubmit(String query) {
			Log.d(TAG, "begin search");
    ...
    protected void setMainContentView() {
        ...
        setContentView(R.layout.search_main); //加载搜索结果显示界面
        mResultView = (TextView) findViewById(R.id.search_result); //搜索界面个数
    }
    ...
    /**
     * 点了searchView中的listView 执行相应的操作
     *
