...
public class KeyguardViewMediator extends SystemUI {
    ...
    private void setup() {
        ...
        //shuyong add for hall begin
        filter.addAction(HALL_STATE_CHANGED_BROADCAST);
        //shuyong add for hall end
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            /// @}
            //shuyong for hall begin
            else if(HALL_STATE_CHANGED_BROADCAST.equals(action)){            
                boolean bClosed = intent.getBooleanExtra(HALL_STATE_CHANGED_EXTRA, false);
                ...
                if(bClosed){
                    synchronized (KeyguardViewMediator.this) {                                
                        if(TelephonyManager.getDefault().getCallState() == TelephonyManager.CALL_STATE_IDLE                                                                     
                            && mExternallyEnabled) {                	        		
                                doKeyguardLocked(null);			                  		
                                mStatusBarKeyguardViewManager.showHall();
                        }                        
                    }
                }else{
                    mStatusBarKeyguardViewManager.hideHall();
                    if(0 == mLockPatternUtils.getActivePasswordQuality()){				
                        keyguardDone(true, true);			
                    }
                }                
            }
            //shuyong for hall end
        }
    };
    ...
    /// @}
    // shuyong add for hall begin
    private static final String HALL_STATE_CHANGED_BROADCAST = "com.kaka.timer.action.HALL_STATE_CHANGED";
    private static final String HALL_STATE_CHANGED_EXTRA = "malata_hall_state";
    // shuyong add for hall end
