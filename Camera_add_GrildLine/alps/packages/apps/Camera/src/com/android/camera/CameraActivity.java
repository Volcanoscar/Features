...
//        setOritationTag(lock, orientationNum);
    ...
    public void onCameraPreferenceReady() {
        ...
        String value = mISettingCtrl.getListPreference(SettingConstants.KEY_GRID_LINE).getValue();
        mPreviewFrameLayout.showGridLine("on".equals(value));
    ...
    private SettingManager.SettingListener mSettingListener = new SettingManager.SettingListener() {
        ...
        public void onSharedPreferenceChanged(ListPreference preference) {
            ...
            if (preference != null) {
                ...
                /*liuhao for POPLUSVNMS-73 grid line 20151222 begin*/
                if (SettingConstants.KEY_GRID_LINE.equals(settingKey)) {
                    mPreviewFrameLayout.showGridLine("on".equals(value));
                }
                /*liuhao for POPLUSVNMS-73 grid line 20151222 end*/
            }
    ...
    private void installIntentFilter() {
        ...
        /*liuhao add for LFZS-55 20150718 begin*/
        ...
        /*liuhao add for LFZS-55 20150718 end*/
    }
    ...
    private boolean isFastAfDisabled() {
        Log.i(TAG, "isFastAfDisabled" + getListPreference(SettingConstants.KEY_FAST_AF));
        if (getListPreference(SettingConstants.KEY_FAST_AF) == null) {
            return true;
        } else if ("off".equals(getListPreference(SettingConstants.KEY_FAST_AF).getValue())) {
            return true;
        } else {
            return false;
        }
    ...
    private final BroadcastReceiver mBatteryReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_BATTERY_CHANGED.equals(action)) {
                String flashMode = mISettingCtrl.getSettingValue(SettingConstants.KEY_FLASH);
                if (flashMode != null && !flashMode.equals("off")) {
                    int level = (int) (100f * intent.getIntExtra("level", 0) / intent.getIntExtra(
                            "scale", 100));
                    if (level <= 15 && !mShowedToast && isFullScreen()) {
                        mShowedToast = true;
                        mCameraAppUi.showToast(R.string.mlt_flash_low_power_tips);
                    }
                }
            }
        }
