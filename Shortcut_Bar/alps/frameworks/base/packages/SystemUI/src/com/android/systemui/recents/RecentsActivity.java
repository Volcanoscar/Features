...
public class RecentsActivity extends Activity implements RecentsView.RecentsViewCallbacks,
    ...
    /*liuhao for One click removal 20150828 begin*/
    private ImageView mOneClickRemoval;
    /*liuhao for One click removal 20150828 end*/
        ...
        public void run() {
            ...
            if (mLaunchIntent != null) {
                ...
                if (FLOAT_WINDOW_SUPPORT) {
                    Log.d(TAG, "Warning: not common feature!!!!!!!!!!!!!!!!!");
                    finish();
    ...
    void updateRecentsTasks(Intent launchIntent) {
        ...
        if (mConfig.launchedToTaskId != -1) {
            if (DEBUG) {
                Log.d(TAG, "updateRecentsTasks:launchedToTaskId = " + mConfig.launchedToTaskId);
            }
            for (int i = 0; i < taskStackCount; i++) {
                ...
                // If the launchedToTask is excluded from recent,
                // the task can't be found in the task list
                if (foundTarget == false) {
                    for (int j = taskCount - 1; j >= 0; j--) {
                        Task t = tasks.get(j);
                        if (t.key.id != -1) {
                            t.isLaunchTarget = true;
                            mConfig.launchedToTaskId = t.key.id;
                            if (DEBUG) {
                                Log.d(TAG, "updateRecentsTasks:update launchedToTaskId = " +
                                    mConfig.launchedToTaskId + " TASK = " +
                                    t.key.toString());
                            }
                            break;
                        }
        ...
        if (mConfig.launchedWithNoRecentTasks) {
            ...
            /*liuhao for One click removal 20150828 begin*/
            if (FeatureOptionUtils.isOneClickRemovalSupport()) {
                mOneClickRemoval.setVisibility(View.GONE);
            }
            /*liuhao for One click removal 20150828 end*/
        } else {
            if (mEmptyView != null) {
            ...
            /*liuhao for One click removal 20150828 begin*/
            if (FeatureOptionUtils.isOneClickRemovalSupport()) {
                mOneClickRemoval.setVisibility(View.VISIBLE);
            }
            /*liuhao for One click removal 20150828 end*/
        }
    ...
    boolean dismissRecentsToFocusedTaskOrHome(boolean checkFilteredStackState) {
        ...
        if (ssp.isRecentsTopMost(ssp.getTopMostTask(), null)) {
            ...
            if (mConfig.launchedFromHome) {
                if (DEBUG) {
                    Log.d(TAG, "dismissRecentsToFocusedTaskOrHome: launchedFromHome");
                }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        /*liuhao for One click removal 20150828 begin*/
        if (FeatureOptionUtils.isOneClickRemovalSupport()) {
            mOneClickRemoval = (ImageView) findViewById(R.id.one_click_removal);
            mOneClickRemoval.setClickable(true);
            mOneClickRemoval.setOnClickListener(new View.OnClickListener() {
    ...
    public void onEnterAnimationTriggered() {
        ...
        if (mConfig.searchBarAppWidgetId >= 0) {
            ...
            ctx.postAnimationTrigger.addLastDecrementRunnable(new Runnable() {
                @Override
                public void run() {
                    /// M: Add log for EnterRecentsAnimation @{
                    if (DEBUG) {
                        Log.d(TAG, "onEnterAnimationTriggered postAnimationTrigger run()");
                    }
                    /// M: Add log for EnterRecentsAnimation @}
