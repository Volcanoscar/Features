...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        loadSettings(db);
        setDefaultGesture();  //caoqiaofeng add MTSFEFL-14 20150327
    }
//caoqiaofeng add MTSFEFL-14 20150327 begin
    ...
    //aoran add for L506_BD_ZT,begin.
    ...
    //aoran add for L506_BD_ZT,end.
	
   public Context getSettingContext(){
            Context contextSetting = null;
        try {
            return mContext.createPackageContext("com.android.settings", 0x2);            
    ...
    public void setDefaultGesture(){  
              Context ctx = getSettingContext();
              if(null == ctx) return;

	    //aoran modify for L506_BD_ZT,begin.

		//M	  
              SharedPreferences pre = ctx.getSharedPreferences(GESTURES_SETTINGS_M, /*MODE_WORLD_READABLE*/1);
              String appName = pre.getString("app_name", "");			  
              if(appName.isEmpty()){	
                  saveSelectedAPP(ctx, GESTURES_SETTINGS_M, "com.android.music", "com.android.music.MusicBrowserActivity", "Music");
              }

		//E	  
              pre = ctx.getSharedPreferences(GESTURES_SETTINGS_E, /*MODE_WORLD_READABLE*/1);
              appName = pre.getString("app_name", "");			  
              if(appName.isEmpty()){	  	
                  //Old: saveSelectedAPP(ctx, GESTURES_SETTINGS_E, "com.android.browser", "com.android.browser.BrowserActivity", "Browser");
                  //caoqiaofeng add LFZS-101 20150716
                  saveSelectedAPP(ctx, GESTURES_SETTINGS_E, "com.android.browser", "com.android.browser.BrowserActivity", "Browser");
              }

	    //C
	          pre = ctx.getSharedPreferences(GESTURES_SETTINGS_C, /*MODE_WORLD_READABLE*/1);
              appName = pre.getString("app_name", "");
              if(appName.isEmpty()){
                  saveSelectedAPP(ctx, GESTURES_SETTINGS_C, "com.android.gallery3d", "com.android.camera.CameraLauncher", "Camera");
              }

		//UP
		      pre = ctx.getSharedPreferences(GESTURES_SETTINGS_UP, /*MODE_WORLD_READABLE*/1);
              appName = pre.getString("app_name", "");
              if(appName.isEmpty()){
                  saveSelectedAPP(ctx, GESTURES_SETTINGS_UP, null, null, "unlock");
              }

		//O
		      pre = ctx.getSharedPreferences(GESTURES_SETTINGS_O, /*MODE_WORLD_READABLE*/1);
              appName = pre.getString("app_name", "");
              if(appName.isEmpty()){
                  saveSelectedAPP(ctx, GESTURES_SETTINGS_O, "com.android.mms", "com.android.mms.ui.ConversationList", "Messaging");
              }

		//W
		      pre = ctx.getSharedPreferences(GESTURES_SETTINGS_W, /*MODE_WORLD_READABLE*/1);
              appName = pre.getString("app_name", "");
              if(appName.isEmpty()){
                  saveSelectedAPP(ctx, GESTURES_SETTINGS_W, "com.android.contacts", "com.android.contacts.activities.PeopleActivity", "Contacts");
              }

		//S
		      pre = ctx.getSharedPreferences(GESTURES_SETTINGS_S, /*MODE_WORLD_READABLE*/1);
              appName = pre.getString("app_name", "");
              if(appName.isEmpty()){
                  saveSelectedAPP(ctx, GESTURES_SETTINGS_S, "com.android.settings", "com.android.settings.Settings", "Settings");
              }

		//V
		      pre = ctx.getSharedPreferences(GESTURES_SETTINGS_V, /*MODE_WORLD_READABLE*/1);
              appName = pre.getString("app_name", "");  			  
              if(appName.isEmpty()){	          
                  saveSelectedAPP(ctx, GESTURES_SETTINGS_V, "com.android.dialer", "com.android.dialer.DialtactsActivity", "Phone");
              }

		//Z
		      pre = ctx.getSharedPreferences(GESTURES_SETTINGS_Z, /*MODE_WORLD_READABLE*/1);
              appName = pre.getString("app_name", "");  			  
              if(appName.isEmpty()){	          
                  saveSelectedAPP(ctx, GESTURES_SETTINGS_Z, "com.mlt.zelta", "com.mlt.zelta.ShowActivity", "Zelta L1");
              }
	    //aoran modify for L506_BD_ZT,end.

    }
	
    public void saveSelectedAPP(Context ctx, String type, String pkg, String cls, String pName){
        SharedPreferences.Editor editor = ctx.getSharedPreferences( type, /*MODE_WORLD_WRITEABLE*/2).edit();
    ...
    //caoqiaofeng add MTSFEFL-14 20150327 end
    ...
    private void loadSystemSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            //caoqiaofeng add SFZT-148 20141106 start
            loadBooleanSetting(stmt, Settings.System.CHARGING_REMIND_STATUS,
                     R.bool.def_charging_remind_status); 

            loadBooleanSetting(stmt, Settings.System.LOW_POWER_REMIND_STATUS,
                     R.bool.def_low_power_remind_status); 

            loadBooleanSetting(stmt, Settings.System.NOTIFICATION_REMIND_STATUS,
                     R.bool.def_notification_remind_status);                    
            //caoqiaofeng add SFZT-148 20141106 end    

            //caoqiaofeng add MTSFEFL-14 20150323 start
     	     loadBooleanSetting(stmt, Settings.System.GLOVE,
		      R.bool.def_glove);

	    loadBooleanSetting(stmt, Settings.System.GESTURE,
		      R.bool.def_gesture);
            //caoqiaofeng add MTSFEFL-14 20150323 end
            loadDefaultHapticSettings(stmt);
