...
PRODUCT_PACKAGES += e2fsck_static
...
ifeq ($(strip $(MTK_MULTI_LOG_SD)), true)
  PRODUCT_PROPERTY_OVERRIDES += ro.multi_log_feature=1
else
  PRODUCT_PROPERTY_OVERRIDES += ro.multi_log_feature=0
endif
...
ifneq ($(strip $(MTK_ECCCI_C2K)),yes)
ifeq ($(strip $(MTK_C2K_SUPPORT)),yes)
PRODUCT_COPY_FILES += device/mediatek/mt6755/init.via.rc:root/init.via.rc
PRODUCT_COPY_FILES += device/mediatek/mt6755/meta_init.via.rc:root/meta_init.via.rc
endif
endif

ifeq ($(strip $(MTK_SHARED_SDCARD)),yes)
ifeq ($(strip $(MTK_2SDCARD_SWAP)),yes)
PRODUCT_COPY_FILES += device/mediatek/mt6755/init.ssd_nomuser.rc:root/init.ssd_nomuser.rc
else
PRODUCT_COPY_FILES += device/mediatek/mt6755/init.ssd.rc:root/init.ssd.rc
endif
else
PRODUCT_COPY_FILES += device/mediatek/mt6755/init.no_ssd.rc:root/init.no_ssd.rc
endif

ifeq ($(strip $(MTK_FAT_ON_NAND)),yes)
PRODUCT_COPY_FILES += device/mediatek/mt6755/init.fon.rc:root/init.fon.rc
endif
PRODUCT_COPY_FILES += device/mediatek/mt6755/init.nvdata.rc:root/init.nvdata.rc
...
ifeq ($(strip $(MTK_GMO_RAM_OPTIMIZE)),yes)
   ifeq ($(strip $(TARGET_PREFER_32_BIT)),yes)
      PRODUCT_PACKAGES += libwebviewchromium_loader
      PRODUCT_PACKAGES += libwebviewchromium_plat_support
      PRODUCT_PACKAGES += libjni_pacprocessor
   endif
endif
