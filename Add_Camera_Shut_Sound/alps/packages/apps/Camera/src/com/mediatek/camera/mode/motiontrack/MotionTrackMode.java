...
import com.mediatek.camera.setting.SettingConstants;// laiyang add for shutter sound 20151103
...
public class MotionTrackMode extends CameraMode implements IFocusManager.FocusListener{
    ...
    private void startNormalCapture() {
        ...
        // laiyang add for shutter sound 20151103 start
        String sVal = mISettingCtrl.getSettingValue(SettingConstants.KEY_SHUTTER_SOUND);
        if(sVal != null && sVal.equals("off")) {
            mICameraDevice.takePicture(null, mRawPictureCallback, mPostViewPictureCallback,
                mJpegPictureCallback);
        } else
        // laiyang add for shutter sound 20151103 end
    ...
    private void onCapture(byte[] data) {
        ...
        if (mIsLongPressed && mCurrentNum == 1) {
            // laiyang modify for shutter sound 20151103 start
            //old:mCaptureSound.play();
            String sVal = mISettingCtrl.getSettingValue(SettingConstants.KEY_SHUTTER_SOUND);
            if(sVal != null && sVal.equals("on")) {
                mCaptureSound.play();
            }
            // laiyang modify for shutter sound 20151103 end
        }
