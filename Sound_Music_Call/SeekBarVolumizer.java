...
public class SeekBarVolumizer implements OnSeekBarChangeListener, Handler.Callback {
    ...
    public SeekBarVolumizer(Context context, int streamType, Uri defaultUri,
        ...
        //chb add for  LFZSF-12 VOICE CALL volume begin
        if(mStreamType == AudioProfileManager.STREAM_VOICE_CALL){
            mOriginalStreamVolume = mSystemVolume;
            Xlog.d(TAG, "initSeekBar sync " + mStreamType + " original Volume to"
                + mSystemVolume);
        }
        //chb add for  LFZSF-12 VOICE CALL volume end
        ...
        if (defaultUri == null) {
            if (mStreamType == AudioProfileManager.STREAM_RING
                    || mStreamType == AudioProfileManager.STREAM_VOICE_CALL) {//chb add for LFZSF-12  Voice Call 2015-9-28
    ...
    public void revertVolume() {
        ...
        // chb add for LFZSF-12 MUSIC volume begin 2015-9-28
        if(mStreamType == AudioProfileManager.STREAM_MUSIC){
            Xlog.d(TAG, "ddd revertVolume " + mStreamType + " ,: "+ mSystemVolume);
            setVolume(mStreamType, mOriginalStreamVolume, false);
        }
        // chb add for LFZSF-12 MUSIC volume end 2015-9-28
