...
public class AudioProfileManager implements IAudioProfileManager {
    ...
    //chb add for LFZSF-12 begin
    public static final int STREAM_MUSIC= AudioManager.STREAM_MUSIC; //add MUSIC volume
    private static final String SUFFIX_MUSIC_VOLUME = "_volume_music"; //add MUSIC volume
    public static final int STREAM_VOICE_CALL = AudioManager.STREAM_VOICE_CALL; //add VOICE CALL volume
    private static final String SUFFIX_VOICE_CALL_VOLUME = "_volume_voice_call"; //add VOICE CALL volume
    ...
    public enum ProfileSettings {
        ...
        //chb add for LFZSF-12 begin
        music_volume,//chb add for MUSIC volume
        voice_call_volume,//chb add for VOICE CALL volume
        //chb add for LFZSF-12 end 2015-9-28
    ...
    public static String getStreamVolumeKey(String profileKey, int type) {
        ...
        switch (type) {
            ...
            case STREAM_VOICE_CALL:
                volumeKey = profileKey + SUFFIX_VOICE_CALL_VOLUME;
                break;
            //chb add for LFZSF-12 end 2015-9-28
            default:
    ...
    public int getStreamVolume(String profileKey, int streamType) {
        ...
        if ((streamType == STREAM_RING) || (streamType == STREAM_NOTIFICATION)
                || (streamType == STREAM_MUSIC)//chb add for LFZSF-12 MUSIC vlume
    ...
    public void setStreamVolume(String profileKey, int streamType, int index) {
        ...
        if ((streamType != STREAM_RING) && (streamType != STREAM_NOTIFICATION)
                && (streamType != STREAM_MUSIC)//chb add for  LFZSF-12 MUSIC volume
                && (streamType != STREAM_ALARM)) {
            Log.e(TAG, "setStreamVolume with unsupport stream type!");
            return;
        }
