...
//zhaoxy 141118 add for LFZS-16 blade view start
import com.android.contacts.common.R;
import com.android.contacts.common.widget.BladeView;
import com.android.contacts.common.list.PinnedHeaderListView;
//zhaoxy 141118 add for LFZS-16 blade view end
...
//zhaoxy add 141118 for LFZS-16 blade view start
    private Boolean isFromMainContact = false;
    private BladeView mBladeView = null;
...
//zhaoxy 141118 add for LFZS-16 blade view end
	...
	/* liuhao add for visitor mode 20141126 begin */
	private boolean mIsInVisitorMode = false;
	/* liuhao add for visitor mode 20141126 end */
    ...
    protected void startLoading() {
        ...
        /* liuhao add for visitor mode 20141126 begin */
        if (SystemProperties.get("ro.mlt.visitor_mode").equals("1")) {
            ...
            if (cr != null) {
                mIsInVisitorMode = (Settings.Global.getInt(cr, "malata_visitor_mode_in", 0) == 1);
            }
            if (mIsInVisitorMode) {
                mAdapter.clearPartitions();
                return;
...
//zhaoxy 141118 add for LFZS-16 blade view start
       // mContactCount = data.getCount();
		if(data!=null){//zhaoxy modify for app force closed when dialpad open without any contact 
        	mContactCount = data.getCount();
		}else{
			mContactCount = 0;
		}
//        if(data.getCount() > 0 && mBladeView != null && !mSearchMode){
//           ensureBladeView();
//        }
       if( mBladeView != null && !mSearchMode){
            ensureBladeView();
       }
//zhaoxy 141118 add for LFZS-16 blade view end
    ...
    private void configureVerticalScrollbar() {
		...
		//zhaoxy add 141223 for LFZS-16 blade view start
		if((mBladeView != null) && (View.VISIBLE == mBladeView.getVisibility()) ){
			hasScrollbar = false ;
		}
		//zhaoxy add 141223 for LFZS-16 blade view end
...
//zhaoxy 141118 add for LFZS-16 blade view start
    private void setBladeViewDisplayEnabled(boolean flag){
    	//Log.v(TAG, "setBladeViewDisplayEnabled,flag:" + flag);
    	if(mBladeView != null){
    		if(flag){
...
//zhaoxy 141118 add for LFZS-16 blade view end
    ...
    protected void setSearchMode(boolean flag) {
        if (mSearchMode != flag) {
            ...
            setBladeViewDisplayEnabled(!mSearchMode);//zhaoxy 141118 add for LFZS-16 blade view
            if (!flag) {
                mDirectoryListStatus = STATUS_NOT_LOADED;
                getLoaderManager().destroyLoader(DIRECTORY_LOADER_ID);
            }
...
//zhaoxy 141118 add for LFZS-16 blade view start
        mBladeView = (BladeView)mView.findViewById(R.id.category);
        setBladeViewDisplayEnabled(!mSearchMode);
//zhaoxy 141118 add for LFZS-16 blade view end
    ...
    //zhaoxy add 141118 for LFZS-16 blade view start
    public void SetIsFromMainContact(boolean flag){
    	isFromMainContact = flag;
    }
	
     public int getSectionForPosition(int position) {
        if ( mAdapter.getIndexer() == null) {
            return 0;
        }
        int headerOffset = 0;
        if(isFromMainContact){
        	headerOffset = 2;
        }
        
        int retVal =  mAdapter.getIndexer().getSectionForPosition(position - headerOffset);
...
//zhaoxy 141118 add for LFZS-16 blade view start
		if (view instanceof PinnedHeaderListView) {
			if (mAdapter != null && !mSearchMode && mContactCount != 0) {
				// disable user profile display 
				int section = getSectionForPosition(firstVisibleItem);
				if (mBladeView != null){
					
				// disable user profile display start 
					 if(section!=-1){  	
					 	mBladeView.setVisibility(View.VISIBLE);
