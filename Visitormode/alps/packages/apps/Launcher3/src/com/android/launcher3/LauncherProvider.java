...
public class LauncherProvider extends ContentProvider {
    ...
    /*liuhao added for visitor mode 20141226 begin*/
    private String V_DATABASE_NAME = "launcher_v.db";
    private DatabaseHelper mVisitorDBHelper;
    private DatabaseHelper mOldDBHelper;
    /*liuhao added for visitor mode 20141226 end*/
    @Override
    public boolean onCreate() {
        ...
        if (SystemProperties.get("ro.mlt.visitor_mode").equals("1")) {
            createVisitorDBHelper();
            mOldDBHelper = mOpenHelper;
        }
        /*liuhao added for visitor mode 20141226 end*/
    ...
    /*liuhao added for visitor mode 20141226 begin*/
    public void reloadLauncherFromDB(boolean isVisitor) {
        if (!SystemProperties.get("ro.mlt.visitor_mode").equals("1")) {
            return;
        }
        if (isVisitor) {
            ...
            createVisitorDBHelper();
            mOldDBHelper = mOpenHelper;
            mOpenHelper = mVisitorDBHelper;
        } else {
            mOpenHelper = mOldDBHelper;
        }
    }
    
    private void copyDatabaseFile() {
        File fromFile = getContext().getDatabasePath(DATABASE_NAME);
        File toFile = getContext().getDatabasePath(V_DATABASE_NAME);
        if (!fromFile.exists() || !fromFile.isFile() || !fromFile.canRead()) {
            return;
        }
        FileUtils.copyFile(fromFile, toFile);
        FileUtils.setPermissions(toFile, 0660, -1, -1);
    }
    
    private void createVisitorDBHelper() {
        String temp = DATABASE_NAME;
        DATABASE_NAME = V_DATABASE_NAME;
        mVisitorDBHelper = new DatabaseHelper(getContext());
        DATABASE_NAME = temp;
    }
    /*liuhao added for visitor mode 20141226 end*/
