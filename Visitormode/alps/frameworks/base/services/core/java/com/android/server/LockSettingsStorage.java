...
class LockSettingsStorage {
    ...
    /*liuhao added for visitor mode 20150112 begin*/
    private static final String LOCK_VISITOR_PATTERN_FILE = "visitor_gesture.key";
    private static final String LOCK_VISITOR_PASSWORD_FILE = "visitor_password.key";
    /*liuhao added for visitor mode 20150112 end*/
    ...
    /*liuhao added for visitor mode 20150112 begin*/
    public byte[] readVisitorModePasswordHash() {
        final byte[] stored = readFile(getLockCredentialFilePathForUser(0, LOCK_VISITOR_PASSWORD_FILE));
        if (stored != null && stored.length > 0) {
            return stored;
        }
        return null;
    ...
    public byte[] readVisitorModePatternHash() {
        final byte[] stored = readFile(getLockCredentialFilePathForUser(0, LOCK_VISITOR_PATTERN_FILE));
        if (stored != null && stored.length > 0) {
            return stored;
        }
        return null;
    ...
    public void writeVisitorModePatternHash(byte[] hash) {
        writeFile(getLockCredentialFilePathForUser(0, LOCK_VISITOR_PATTERN_FILE), hash);
    }

    public void writeVisitorModePasswordHash(byte[] hash, int userId) {
        writeFile(getLockCredentialFilePathForUser(0, LOCK_VISITOR_PASSWORD_FILE), hash);
    }
    /*liuhao added for visitor mode 20150112 end*/
